...
def extract_statistics(cfg, train_set, inliner_classes, E, G):
    ...
    for label, x in data_loader:
        ...
        diff = x.reshape(x.shape[0], -1) - recon_batch.reshape(x.shape[0], -1)
    ...
    difflist = np.concatenate(difflist)
    rlist = np.concatenate(rlist)
    #
    # counts, bin_edges = np.histogram(difflist, bins=10000, density=True)
    # plt.figure(num=None, figsize=(24, 12), dpi=80, facecolor='w', edgecolor='k')
    #
    # plt.xlim(-0.05, 0.05)
    # plt.ylim(0.0, 100)
    # x = np.linspace(-0.05, 0.05, 10000)
    # y = r_pdf(x, bin_edges, counts)
    #
    # plt.plot(x, y, linewidth=2)
    # save_plot(r"Distance,  I^i - \hat{I}^i  $",
    #           'Probability density',
    #           r"PDF of pixel-wise reconstruction error, $p\left(  I^i - \hat{I}^i   \right)$",
    #           cfg.OUTPUT_FOLDER + '/mnist_%s_difflist.png' % ("_".join([str(x) for x in inliner_classes])))
    #
    # with open("difflist_%s.pkl" % ("_".join([str(x) for x in inliner_classes])), "wb") as f:
    #     pickle.dump(difflist, f)
    #
    # exit()

    # with open("rlist_%s.pkl" % ("_".join([str(x) for x in inliner_classes])), "wb") as f:
    #     pickle.dump(rlist, f)

    counts, bin_edges = np.histogram(rlist, bins=30)
    counts[counts == 0] = 1
    d = bin_edges[1:] - bin_edges[:-1]
    counts = counts.astype(np.float32) / (counts * d).sum()

    # Find best fit distribution
    r_params = st.genlogistic.fit(rlist, floc=0)

    # x = np.linspace(0.0, 8.0, 10000)
    # n, loc, scale = mvgn.fit(rlist, floc=0)
    # print(n, scale)
    ...
    #           cfg.OUTPUT_FOLDER + '/mnist_%s_reconstruction_error.png' % ("_".join([str(x) for x in inliner_classes])))
    # exit()
    ...
    return counts, bin_edges, r_params, gennorm_param
...
def main(folding_id, inliner_classes, ic, total_classes, mul, folds=5, cfg=None):
    ...
    counts, bin_edges, r_params, gennorm_param = extract_statistics(cfg, train_set, inliner_classes, E, G)
    ...
    def run_novely_prediction_on_dataset(dataset, percentage, concervative=False):
        ...
        logC = scipy.special.loggamma(N / 2.0) - np.log(2.0) - (N / 2.0) * np.log(np.pi)

        def logPe_func(x):
            # p_{\|W^{\perp}\|} (\|w^{\perp}\|)
            # \| w^{\perp} \|}^{m-n}
            return logC - (N - 1) * np.log(x), st.genlogistic.logpdf(x, *r_params)
        ...
        for label, x in data_loader:
            ...
            for i in range(x.shape[0]):
                ...
                result.append((logPz, logD, logPe_p1, logPe_p2))
    ...
    percentage = 50
    y_scores_components_v, y_true_v = run_novely_prediction_on_dataset(valid_set, percentage, concervative=True)
    y_scores_components_t, y_true_t = run_novely_prediction_on_dataset(test_set, percentage, concervative=True)

    os.makedirs(os.path.join(cfg.OUTPUT_FOLDER, "dump/"), exist_ok=True)
    with open(os.path.join(cfg.OUTPUT_FOLDER, "dump/precomputed_%d_%d.pkl" % (folding_id, ic)), mode='wb') as f:
        pickle.dump(dict(
            y_scores_components_v=y_scores_components_v,
            y_true_v=y_true_v,
            y_scores_components_t=y_scores_components_t,
            y_true_t=y_true_t
        ), f)
